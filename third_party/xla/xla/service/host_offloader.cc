...
absl::StatusOr<bool> HostOffloader::WalkDownHostMemoryOffloadPaths(
  ...
  while (!queue.empty()) {
    ...
    if (instruction->opcode() == HloOpcode::kCustomCall &&
      ...
      if (instruction == starting_instruction) {
        ...
        dynamic_update_slices_already_handled_.insert(instruction);
      }
  ...
  // Remove MoveToHost custom calls.
        ...
        [&](const Shape& subshape, const ShapeIndex& index) {
          if (subshape.has_layout() &&
            ...
            VLOG(1) << "Host parameter streamed into program with shape: "
                    ...
                    << index.ToString();
            absl::StatusOr<bool> result = WalkDownHostMemoryOffloadPaths(
                InstructionAndShapeIndex(parameter_instruction, index),
                /*insert_copy_before=*/false);
            if (!result.ok()) {
              // TODO: How do I properly get a Status out of ForEachSubshape?
...
absl::StatusOr<bool> HostOffloader::HandleMoveToDeviceCustomCall(
    HloInstruction* custom_call_instruction) {
  // Since we traverse the graph in post-order, we can assume that any host
  // memory offloading has been completely handled. This MoveToDevice custom
  // call can and should be removed.
...
absl::Status HostOffloader::ValidateSliceLeadsToMoveToDeviceCustomCall(
  ...
  std::queue<InstructionAndShapeIndex> queue;
  for (HloInstruction* slice_user : slice->users()) {
    queue.push(InstructionAndShapeIndex(slice_user));
  }
  while (!queue.empty()) {
    InstructionAndShapeIndex instruction_and_shape = queue.front();
    queue.pop();
    ...
    if (!InstructionIsAllowedBetweenDsAndMoveToDevice(current_instruction)) {
      return absl::InvalidArgumentError(absl::StrFormat(
          ...
          "has an invalid instruction between DynamicSlice/Slice and the "
          "MoveToDevice custom call.",
...
absl::Status HostOffloader::CreateAllocateBufferForDynamicUpdateSlice(
  ...
  if (dynamic_update_slices_already_handled_.find(dynamic_update_slice) !=
      dynamic_update_slices_already_handled_.end()) {
      ...
      for (HloInstruction* caller : callers) {
        if (caller->opcode() == HloOpcode::kWhile) {
          ...
          SetMemorySpace(ShapeUtil::GetMutableSubshape(
                             ...
                             instruction_and_shape.shape_index),
                         kHostMemorySpaceColor);
          // TODO: We should also walk further down the graph and set the memory
          // spaces of all uses too. This includes verifying that no compute is
          // done on the buffer. Another, better way, to do this, is to walk
          // down the graph starting from the newly created AllocateBuffer and
          // set everything visited as host memory space.
        }
      }
    }
  ...
  dynamic_update_slices_already_handled_.insert(dynamic_update_slice);
  return OkStatus();
}
